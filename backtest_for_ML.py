...
from indicators import is_hammer, is_doji, candle_pattern, candle_params


def save_signal(signal):
    '''Save trade statistics to a file from the trade dictionary 'in_trade' '''
    
    fname = 'buy_signals_backtest_ML_July-Aug2020.dat'
                    ...
                    if stochRSI_condition:
                        ...
                        for j in range(start_j, end_j):
                            ...
                            if final_stoch_condition  and price_cond :
                                ...
                                # Stoch RSI for 15 minute intervals
                                stoch_k_15m = fastk_curr.iloc[-1]
                                slowd_15m = slowd_curr.iloc[-1]
                                # Derivatives (slopes) of slowd and fastk:
                                dfastk_15m = (stoch_k_15m - fastk_curr.iloc[-2])/15
                                dslowd_15m = (slowd_15m - slowd_curr.iloc[-2])/15
                                # Derivatives of Bollinger Bands:
                                dlower = (lower.iloc[i-1] - lower.iloc[i-2])/15
                                dmiddle = (middle.iloc[i-1] - middle.iloc[i-2])/15
                                dupper = (upper.iloc[i-1] - upper.iloc[i-2])/15
                                dranging = (ranging.iloc[i] - ranging.iloc[i-1])/15
                                # Exponential moving averages:
                                ema10 = ta.trend.EMAIndicator(df_ohlc['close'], n=10).ema_indicator()
                                ema200 = ta.trend.EMAIndicator(df_ohlc['close'], n=200).ema_indicator()     
                                # And their derivatives:
                                dema10 = (ema10.iloc[i-1] - ema10.iloc[i-2])/15
                                dema200 = (ema200.iloc[i-1] - ema200.iloc[i-2])/15
                                ...
                                profit_s15, elapsed_s15, min_price_s15, max_price_s15 = evaluate(time_curr_1m, price_curr, 
                                                                                                 df_ohlc_1m, 
                                ...
                                if profit == None: 
                                    ...
                                    signal = {'time_curr':f'{time_curr_1m}', 'symbol':f'{symbol}','price':f'{price_curr:.8f}', 'pattern':f'{pattern}', 
                                          'origin':f'{signal_origin}', 'ranging': f'{ranging.iloc[i]:.3f}', 'd_ranging':f'{dranging:.3f}',
                                          'lower':f'{lower.iloc[i-1]:.8f}', 'upper':f'{upper.iloc[i-1]:.8f}', 'middle':f'{middle.iloc[i-1]:.8f}',
                                          'd_lower':f'{dlower:.8f}', 'd_upper':f'{dupper:.8f}', 'd_middle':f'{dmiddle:.8f}',
                                          'ema_10':f'{ema10.iloc[i-1]:.8f}', 'ema_200':f'{ema200.iloc[i-1]:.8f}',
                                          'd_ema_10':f'{dema10:.8f}', 'd_ema_200':f'{dema200:.8f}',
                                          'k_15':f'{stoch_k_15m:.2f}', 'd_15':f'{slowd_15m:.2f}', 
                                          'd_k_15':f'{dfastk_15m:.2f}', 'd_d_15':f'{dslowd_15m:.2f}',
                                          'stoch_cond_5': f'{final_stoch_condition_5m}','candle_color':f'{last_candle_color}',
                                          'vol_inc':f'{vol_pm_inc:.3f}', 'dist_to_BB':f'{dist_to_BB:.3f}',
                                          'profit':f'{profit:.2f}', 'elapsed':f'{elapsed:.1f}', 
                                          'min_price':f'{min_price:.2f}', 'max_price': f'{max_price:.2f}',
                                          'profit_s15':f'{profit_s15:.2f}','elapsed_s15':f'{elapsed_s15:.1f}',
                                          'min_price_s15':f'{min_price_s15:.2f}', 'max_price_s15':f'{max_price_s15:.2f}' }
